...
const ProductsDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (apiRes.ok) {
        setForm({ name: "", price: "", category: "", images: [], description: "", size: "", rooms: "", baths: "" });
        setEditingId(null);
        fetchProducts();
  ...
  const handleDelete = async (id) => {
    ...
    await fetch("/api/product", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
    fetchProducts();
  };
  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6 space-y-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-700">{editingId ? "تعديل المنتج" : "إضافة منتج جديد"}</h2>
        ...
        <button type="submit" className="w-full bg-blue-500 hover:bg-blue-600 text-white p-3 rounded">{editingId ? "تعديل المنتج" : "إضافة المنتج"}</button>
      </form>
